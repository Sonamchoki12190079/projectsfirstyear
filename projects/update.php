...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($enrollment_No_err) && empty($percentage_err)){
        ...
        $sql = "UPDATE students SET name=?, enrollment_No=?, percentage=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_enrollment_No, $param_percentage, $param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($enrollment_No_err)) ? 'has-error' : ''; ?>">
                            <label>Enrollment_No</label>
                            <textarea name="enrollment_No" class="form-control"><?php echo $enrollment_No; ?></textarea>
